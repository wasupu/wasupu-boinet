...
if [[ $# -ne 6 ]]; then
    echo "Usage: ./deploy.sh <AWS_ACCESS_KEY> <AWS_SECRET_KEY> <AWS_DEFAULT_REGION> <AWS_DOCKER_REGISTRY> <STREAM_SERVICE_API_KEY> <STREAM_SERVICE_NAMESPACE>"
    exit 1
fi
AWS_ACCESS_KEY="$1"
AWS_SECRET_KEY="$2"
...
createEcsTaskDefinition ${TASK_DEFINITION_NAME} ${TASK_DEFINITION_FILE} ${AWS_DOCKER_REGISTRY} ${AWS_LOGS_GROUP} ${AWS_DEFAULT_REGION} ${STREAM_SERVICE_API_KEY} ${STREAM_SERVICE_NAMESPACE}
